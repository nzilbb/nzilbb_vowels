...
#' @param cutoff the cutoff value for interpretation of the PC. The default
#'   value, 50, means we ignore contributions until they cumulatively account
#'   for more than 50% of the PC.
...
pca_contrib_plot <- function(pca_object, pc_no=1, cutoff=50) {
  ...
  loadings <- as_tibble(onze_pca$rotation[,pc_no], rownames="variable") %>%
    ...
    mutate(
      ...
      highlight = cumulative_contribution > cutoff
    )
