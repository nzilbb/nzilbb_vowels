...
In @fig-3d, the most obvious thing to note is that [dress]{.smallcaps} and
...
of points. In @fig-3d we depict this cloud using both spatial position and
colour.

```{r fig-centre, fig.cap = "@fig-3d with centre of point cloud added."}
...
@fig-centre has a large 'X' indicating the centre point, or mean value, for each
...
@fig-PC1 shows this line. It is clear that it passes through [dress]{.smallcaps}
...
The slightly thinner line in @fig-PC2 represents PC2. If we look at the end of
...
didn't have to decide on a direction. For instance, in @fig-PC2, we
...
@fig-biplot shows the **loadings** of the original variable for PC1
...
How do you read @fig-biplot? The arrows show which way the original
...
In @fig-biplot we have achieved a representation of speakers in terms of
two variables with a clear linguistic meaning: overall vocal tract length and
position in the NZE short front vowel shift. When we started, in @fig-3d,
...
@fig-biplot-sup is exactly what we expect. Low PC2 values correspond to
...
@fig-qb1-cortests shows that there are many more high magnitude correlations in our
...
@fig-screeplot is an example of a "screeplot". Various methods are out there for
...
@fig-screeplot-2 introduces two changes to what we saw in @fig-screeplot. First,
...
@fig-contrib-nocut shows the loadings for PC1 with and without a cutoff value at
...
When we look at @fig-pc1-loadings, we see black indications of the loading and
...
explore the consequences of alternative rules for including variables.^[In @fig-pc1-loadings, we might be tempted to exclude all variables whose red
bars overlap with the blue. In many cases, we have found this rule to be too
conservative.]

To repeat, because this can be unclear: we take plots like @fig-pc1-loadings
...
So what does @fig-pc1-loadings *mean*? We can read the relationships off the
...
The overall impression in @fig-pc1-vs is that higher values of PC1 lead to more
...
Now we repeat @fig-pc1-vs, and see that the loadings have reversed.

```{r fig-pc1-vs-flipped, fig.cap = "PC1 loadings represented in vowel space. As PC1 scores increase, a speakers vowels tend to move in the direction of the arrows. Flipped from @fig-pc1-vs.", fig.height = 8}
pc1_vs <- plot_pc_vs(
...
pc2_vs
```
As in [@brandSystematicCovariationMonophthongs2021], @fig-pc2-vs shows a pattern
