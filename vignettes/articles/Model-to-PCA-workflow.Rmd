...
author: "Joshua Wilson Black, Lynn Clark"
resource_files:
  - data/QB1_anon.rds
  - data/QB1_preds.rds
  - data/QB1_intercepts.rds
pkgdown:
  as_is: true
bibliography:
  - references.bib
...
**This material slightly expands on @wilsonblackUsingPrincipalComponent2023.**
**For almost all purposes, it would be better to cite that article than this** **page.**
...
In Figure \@ref(fig:fig-3d), the most obvious thing to note is that [dress]{.smallcaps} and
...
of points. In Figure \@ref(fig:fig-3d) we depict this cloud using both spatial position and
colour.
...
Figure \@ref(fig:fig-centre) has a large 'X' indicating the centre point, or mean value, for each
...
Figure \@ref(fig:fig-PC1) shows this line. It is clear that it passes through [dress]{.smallcaps}
...
The slightly thinner line in Figure \@ref(fig:fig-PC2) represents PC2. If we look at the end of
...
didn't have to decide on a direction. For instance, in Figure \@ref(fig:fig-PC2), we
...
Figure \@ref(fig:fig-biplot) shows the **loadings** of the original variable for PC1
...
How do you read Figure \@ref(fig:fig-biplot)? The arrows show which way the original
...
In Figure \@ref(fig:fig-biplot) we have achieved a representation of speakers in terms of
two variables with a clear linguistic meaning: overall vocal tract length and
position in the NZE short front vowel shift. When we started, in Figure \@ref(fig:fig-3d),
...
Figure \@ref(fig:fig-biplot-sup) is exactly what we expect. Low PC2 values correspond to
...
```{r, include = FALSE}
QB1_preds <- read_rds('data/QB1_preds.rds')
```
...
```{r, include = FALSE}
QB1_intercepts <- read_rds('data/QB1_intercepts.rds')
```
...
Figure \@ref(fig:fig-qb1-cortests) shows that there are many more high magnitude correlations in our
...
Figure \@ref(fig:fig-screeplot) is an example of a "screeplot". Various methods are out there for
...
Figure \@ref(fig:fig-screeplot-2) introduces two changes to what we saw in Figure \@ref(fig:fig-screeplot). First,
...
Figure \@ref(fig:fig-contrib-nocut) shows the loadings for PC1 with and without a cutoff value at
...
When we look at Figure \@ref(fig:fig-pc1-loadings), we see black indications of the loading and
...
explore the consequences of alternative rules for including variables.^[In Figure \@ref(fig:fig-pc1-loadings), we might be tempted to exclude all variables whose red
bars overlap with the blue. In many cases, we have found this rule to be too
conservative.]

To repeat, because this can be unclear: we take plots like Figure \@ref(fig:fig-pc1-loadings)
...
So what does Figure \@ref(fig:fig-pc1-loadings) *mean*? We can read the relationships off the
...
The overall impression in Figure \@ref(fig:fig-pc1-vs) is that higher values of PC1 lead to more
...
Now we repeat Figure \@ref(fig:fig-pc1-vs), and see that the loadings have reversed.
...
pc2_vs
```
As in [@brandSystematicCovariationMonophthongs2021], Figure \@ref(fig:fig-pc2-vs) shows a pattern
