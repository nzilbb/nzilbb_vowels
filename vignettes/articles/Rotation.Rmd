...
# Comparing PCA across data sets.
...
The trajectories in \@ref(fig:fig-qb-preds) are in line with those found in
...
```{r pc1-pc2-loadings, fig.cap="Loadings for PC1 and PC2 across ONZE and QB."}
...
Spend some time peering at \@ref(fig:fig-pc1-pc2-loadings). You will start to see patterns
...
\@ref(fig:fig-pc1-pc2-loadings), which uses only a subset of the QB data.]
...
around the plots of variance explained above. If, as in \@ref(fig:fig-onze-var),
...
The moral of \@ref(fig:fig-bootstrap-plot) is that the same *patterns* are pretty 
...
not overlap for *both* corpora (i.e., both \@ref(fig:fig-onze-var) and \@ref(fig:fig-qb-var) show
...
Let's repeat \@ref(fig:fig-pc1-pc2-loadings) with `prcomp()`.
...
\@ref(fig:fig-rot-loadings) roughly aligns the first component in ONZE with the first
...
```{r fig-rot-flip-loadings, fig.cap="ONZE and QB loadings for components 1 and 2, ONZE rotated and then flipped on Component 2."}
...
\@ref(fig:fig-onze-var).] We will again rotate ONZE to match QB (but we could
...
loadings_to_plot <- bind_rows(
  "QB" = as_tibble(qb_pca$rotation, rownames="variable"),
  "ONZE" = as_tibble(onze_proc$rotation, rownames="variable"),
  .id = "Corpus"
)
...
How does this compare to the manual rotation we tried above (\@ref(fig:fig-rot-flip-loadings)?
```{r}
loadings_to_plot <- bind_rows(
...
```{r, include = FALSE}
QB1_ints <- read_rds(file.path(me, 'data/QB1_scores_anon.rds')) |> 
  select(-contains("PC"))
QB2_ints <- read_rds(file.path(me, 'data/QB2_scores_anon.rds'))|> 
  select(-contains("PC"))
```
...
## Procrustes rotation for improved bootsrapping

We can use Procrustes rotation in `pca_test()` for an alternative source of
confidence bars.

This is an approach sometimes used in Multidimensional Scaling (MDS).

The fundamental idea is the same as for the case of comparing across two datasets,
for each bootstrap sample is a distinct dataset.
